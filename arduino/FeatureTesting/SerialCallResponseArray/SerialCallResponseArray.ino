...
//    thirdSensor = analogRead(2) / 4;
    ...
    for(int i = 0; i < sizeof(knobs)/sizeof(int); i++){
      analogRead(i);
      knobs[i] = analogRead(i);
    }
    byte bytes[sizeof(knobs)/2];
    for(int i = 0; i < sizeof(bytes); i+=2){
      bytes[i] = highByte(knobs[i]);
      bytes[i+1] = lowByte(knobs[i]);
    }
    // send sensor values:
...
//    Serial.write(thirdSensor);
  Serial.print("value;: ");
  Serial.print(knobs[0]);
  Serial.print(" bin: ");
  Serial.print(knobs[0], BIN);
  Serial.print(" high: ");
  Serial.print(highByte(knobs[0]), BIN);
  Serial.print(" high byte: ");
  Serial.print(bytes[0], BIN);
  Serial.print(" low: ");
  Serial.println(lowByte(knobs[0]), BIN);
